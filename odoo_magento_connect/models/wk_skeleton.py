...
class WkSkeleton(models.TransientModel):
    ...
    def create_order_invoice(self, orderId, ecommerceInvoiceId=False):
        ecommerceInvoiceId = ecommerceInvoiceId or orderId.get(
            "ecommerceInvoiceId", False)
        orderId = orderId.get("orderId", False)
        ctx = dict(self._context or {})
        ...
        if all(key in ctx for key in ('invoice_date', 'magento')) and 'invoice_id' in res:
            accountObj = self.env['account.move'].browse(res['invoice_id'])
            accountObj.write({'invoice_date': ctx.get('invoice_date')})
        return res
