...
class MagentoSynchronization(models.TransientModel):
    ...
    def _export_specific_product(self, vrntObj, templateSku, url, token):
        ...
        if vrntObj:
            ...
            if vrntObj.product_template_attribute_value_ids:
                for valueObj in vrntObj.product_template_attribute_value_ids:
                    ...
                    mageValueId = attrValMapModel.search(
                        [('name', '=',
                          valueObj.product_attribute_value_id.id)],limit=1).mage_id or 0
            ...
            priceExtra = vrntObj.price_extra or 0.00
            getProductData.update(
                attribute_set_id=mageSetId,
                type_id=prodtype,
    ...
    def _update_specific_product(self, prodMapObj, url, token):
        ...
        if prodObj and mageProdId:
            priceExtra = prodObj.price_extra
            custom_attributes = []
            if prodObj.product_template_attribute_value_ids:
                for valueObj in prodObj.product_template_attribute_value_ids:
                    mageAttributeCode = attrMapModel.search(
                        [('name', '=', valueObj.attribute_id.id)],limit=1).mage_attribute_code or False
                    mageValueId = attrValMapModel.search(
                        [('name', '=',
